 ...
 * @param  {string} installString   pip install argument for the plugin
 * Custom for schistosomiasis:
 * @param  {string} notebookPath   the plugin's companion notebook location
 */
function storePluginMetadataSync(
  micromamba, pluginEnvPrefix, packageName, installString, notebookPath
) {
  ...
  settingsStore.set(
    `plugins.${pluginID.replaceAll('.', '_')}`,
    {
      ...
      env: pluginEnvPrefix,
      version: version,
      notebook_path: notebookPath, // custom schisto-invest only
    }
  );
}
  ...
  ipcMain.handle(
    ...
    async (event, url, revision, localPath) => {
      try {
        ...
        // Unique / custom to schisto-invest and the schisto plugin only
